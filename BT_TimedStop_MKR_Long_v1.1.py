        ...
        if daily_time_frame == True:
            ...
            if order == 'long' and working == 0:
                ...
                while (i + j) < len(df):
                    ...
                    df.at[i + j, 'working'] = 1
                    j += 1
            # elif order == 'short' and working == 0:
            #     j = 1
            #     while (i + j) < len(df):
            #         # First check if close at day open
            #         if price - tp >= df.at[i + j, 'Open']:
        ...
        if daily_time_frame == False:
            ...
            if order == 'long' and working == 0 and (df.at[i, 'Datetime_x'].time() >= start_time) and (df.at[i, 'Datetime_x'].time() <= end_time):
                ...
                while (i + j) < len(df):
                    ...
                    df.at[i + j, 'working'] = 1
                    j += 1
            # elif order == 'short' and working == 0 and (df.at[i, 'Datetime_x'].time() >= start_time) and (df.at[i, 'Datetime_x'].time() <= end_time):
            #     j = 1
            #     while (i + j) < len(df):
            #         if (df.at[i+j, 'Datetime_x'].time() >= start_time) and (df.at[i+j, 'Datetime_x'].time() <= end_time):
            #             # This case handles trades open at start time.
...
timeframes = [df_full]
close_on_opp_list = [False]
close_on_day_end_list = [False]
atr_tp_list = [2.0, 2.5, 2.75, 3.0, 3.25, 3.5]
atr_sl_list = [4.0, 4.5, 5.0, 5.5, 6.0, 6.5]
timeout_list = [4, 5, 6, 7, 8]
length_list = [2, 5, 9, 25, 50, 100]
bandwidth_list = [7, 8, 9, 10, 11, 12]
diff_threshold_list = [0]
consec_list = [1, 2]
start_time_list = ['09:30:00']
kernel_name_list = ["silverman"]
#ma_list = ['none', '50-20','100-50','10-5','30-10']
ma_list = ['none']
#stoch_list = ['none', '5-3', '10-5', '14-3', '20-8', '8-3', '18-7', '15-6']
stoch_list = ['none']
...
if USE_ITERTOOLS == True:
    ...
    CSV_NAME = 'mkr_timedStop_2015_market_1hour_long_opt.csv'
